    ...
    return (
        <div className="bg-white  shadow-lg rounded-lg p-4 mt-4">
            <div className="flex items-center">
                <h2 className="text-lg font-semibold flex-grow text-black ">Location Details</h2>
                {places.length > 3 && (
            ...
            <div className={`space-y-4 transition-all duration-500 ${showMore ? 'max-h-[5000px]' : 'max-h-[300px]'} overflow-hidden`}>
                {places?.slice(0, showMore ? places.length : 3).map((place: Place) => (
                    <div key={place.cid} className="bg-gray-100  rounded-lg p-4">
                        ...
                        <p className="text-gray-600text-sm mb-2">Category: {place.category}</p>
                        {place.phoneNumber && (
